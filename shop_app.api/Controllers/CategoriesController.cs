...
namespace shop_app.api.Controllers
{
    ...
    public class CategoriesController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("Page")]
        public async Task<ActionResult<IEnumerable<CategoryDto>>> GetCategories([FromQuery]int page, [FromQuery]int size) // Query
        {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<CategoryDto>>> GetCategories() // Query
        {
        ...
        public async Task<ActionResult<CategoryDto>> CreateCategory([FromBody] CategoryDto categorydto)
        {
            var response = await _mediator.Send(new SubmitCategoryRequest
            {
                ...
                // Put Seller relation here.
            });
            return this.FromResult(response);
        }

        [HttpPut]
        [Route("Status/activate/{uri}")]
        public async Task<ActionResult<CategoryDto>> ToggleStatus([FromRoute] string uri)
        {
            //TODO: There is performance issue. We are fetching same category twice. We must fetch it once and provide it's guid.
            ...
            if (categoryResponse.Succeed)
            {
                ...
                var statusResponse = await _mediator.Send(
                    new UpdateCategoryRequest { Category = category, CategoryUri = category.Uri});
